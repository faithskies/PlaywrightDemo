...
test('Test Case 1 - WebApp - Implement user authentication - Confirm tags: "Feature" "High Priority”', async ({ page }) => {
  ...
  console.log(allColumnDataByHeader);
  let searchvalue = 'To Do'
  resultFound = await helperFunctions.AllTextSearch(searchvalue, allColumnDataByHeader)
  await expect(resultFound, "Expect searchvalue: " + searchvalue + " to exist in the list which results in resultfound=true").toBe(true);
  ...
  console.log(allTileDataByHeader);
  ...
  for (let i = 0; i < searchTileForValue.length; i++) {
        ...
        await expect(resultFound, "Expect searchvalue: " + searchTileForValue[i] + " to exist in the list which results in resultfound=true" ).toBe(true);
